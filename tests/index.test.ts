...
import { APIPromise } from 'eesuhntest/core/api-promise';

import util from 'node:util';
import Eesuhntest from 'eesuhntest';
import { APIUserAbortError } from 'eesuhntest';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['EESUHNTEST_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Eesuhntest) => {
    ...
    test('debug logs when log level is debug', async () => {
      ...
      const client = new Eesuhntest({ logger: logger, logLevel: 'debug' });
    ...
    test('debug logs are skipped when log level is info', async () => {
      ...
      const client = new Eesuhntest({ logger: logger, logLevel: 'info' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['EESUHNTEST_LOG'] = 'debug';
      const client = new Eesuhntest({ logger: logger });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['EESUHNTEST_LOG'] = 'not a log level';
      const client = new Eesuhntest({ logger: logger });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'EESUHNTEST_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
    ...
    test('client log level overrides env var', async () => {
      ...
      process.env['EESUHNTEST_LOG'] = 'debug';
      const client = new Eesuhntest({ logger: logger, logLevel: 'off' });
  ...
  describe('defaultQuery', () => {
    ...
    test('overriding with `undefined`', () => {
      const client = new Eesuhntest({ baseURL: 'http://localhost:5000/', defaultQuery: { hello: 'world' } });
  ...
  test('explicit global fetch', async () => {
    ...
    const client = new Eesuhntest({ baseURL: 'http://localhost:5000/', fetch: defaultFetch });
  });
  ...
  test('normalized method', async () => {
    ...
    const client = new Eesuhntest({ baseURL: 'http://localhost:5000/', fetch: testFetch });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new Eesuhntest({ baseURL: 'http://localhost:5000/custom/path/' });
    ...
    afterEach(() => {
      process.env['EESUHNTEST_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new Eesuhntest({ baseURL: 'https://example.com' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['EESUHNTEST_BASE_URL'] = 'https://example.com/from_env';
      const client = new Eesuhntest({});
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['EESUHNTEST_BASE_URL'] = ''; // empty
      const client = new Eesuhntest({});
      expect(client.baseURL).toEqual('https://pro-api.coingecko.com/api/v3');
    });

    test('blank env variable', () => {
      process.env['EESUHNTEST_BASE_URL'] = '  '; // blank
      const client = new Eesuhntest({});
      expect(client.baseURL).toEqual('https://pro-api.coingecko.com/api/v3');
    });
    ...
    test('env variable with environment', () => {
      ...
      expect(() => new Eesuhntest({ environment: 'production' })).toThrowErrorMatchingInlineSnapshot(
        `"Ambiguous URL; The \`baseURL\` option (or EESUHNTEST_BASE_URL env var) and the \`environment\` option are given. If you want to use the environment you must pass baseURL: null"`,
      );

      const client = new Eesuhntest({ baseURL: null, environment: 'production' });
      expect(client.baseURL).toEqual('https://pro-api.coingecko.com/api/v3');
    });
  });

  test('maxRetries option is correctly set', () => {
    const client = new Eesuhntest({ maxRetries: 4 });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Eesuhntest({});
    expect(client2.maxRetries).toEqual(2);
  });

  describe('withOptions', () => {
    test('creates a new client with overridden options', () => {
      const client = new Eesuhntest({ baseURL: 'http://localhost:5000/', maxRetries: 3 });
    ...
    test('respects runtime property changes when creating new client', () => {
      const client = new Eesuhntest({ baseURL: 'http://localhost:5000/', timeout: 1000 });
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new Eesuhntest({ timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new Eesuhntest({ fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Eesuhntest({ fetch: testFetch });
