...
import { APIPromise } from '@eesuhn/eesuhntest-typescript/core/api-promise';

import util from 'node:util';
import Eesuhntest from '@eesuhn/eesuhntest-typescript';
import { APIUserAbortError } from '@eesuhn/eesuhntest-typescript';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    test('debug logs when log level is debug', async () => {
      ...
      const client = new Eesuhntest({ logger: logger, logLevel: 'debug', proAPIKey: 'My Pro API Key' });
    ...
    test('default logLevel is warn', async () => {
      const client = new Eesuhntest({ proAPIKey: 'My Pro API Key' });
    ...
    test('debug logs are skipped when log level is info', async () => {
      ...
      const client = new Eesuhntest({ logger: logger, logLevel: 'info', proAPIKey: 'My Pro API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      const client = new Eesuhntest({ logger: logger, proAPIKey: 'My Pro API Key' });
    ...
    test('client log level overrides env var', async () => {
      ...
      process.env['EESUHNTEST_LOG'] = 'debug';
      const client = new Eesuhntest({ logger: logger, logLevel: 'off', proAPIKey: 'My Pro API Key' });
  ...
  describe('baseUrl', () => {
    ...
    test('explicit option', () => {
      const client = new Eesuhntest({ baseURL: 'https://example.com', proAPIKey: 'My Pro API Key' });
    ...
    test('env variable with environment', () => {
      ...
      expect(
        () => new Eesuhntest({ proAPIKey: 'My Pro API Key', environment: 'pro' }),
      ...
      const client = new Eesuhntest({ proAPIKey: 'My Pro API Key', baseURL: null, environment: 'pro' });
      expect(client.baseURL).toEqual('https://pro-api.coingecko.com/api/v3');
    });
  });

  test('maxRetries option is correctly set', () => {
    const client = new Eesuhntest({ maxRetries: 4, proAPIKey: 'My Pro API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Eesuhntest({ proAPIKey: 'My Pro API Key' });
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new Eesuhntest({ proAPIKey: 'My Pro API Key', timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new Eesuhntest({ proAPIKey: 'My Pro API Key', fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Eesuhntest({ proAPIKey: 'My Pro API Key', fetch: testFetch });
